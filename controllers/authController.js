...
export const register = async (req, res) => {
  try {
    ...
    try {
      await sendEmail({
        email,
        subject: "Verify Your Email - ApexRoot Platform",
        message: `<p>Your email verification code is: <strong>${verificationToken}</strong>.</p>\n <p>Please enter this code on the verification page to activate your account.</p>`,
      });
    } catch (Emerror) {
      console.error("Failed to send verification email:", Emerror);
    }
...
export const forgotPassword = async ( req, res ) => {
    try {
        ...
        const resetPasswordExpire = Date.now() + 15 * 60 * 1000; // 15mins
        ...
        try {
            await sendEmail({
                email: user.email,
                subject: "Password Reset - ApexRoot Platform",
                message,
            })
            res.status(200).json({
